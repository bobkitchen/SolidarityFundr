...
class DataImportExport {
    ...
    private func performImport(_ importData: ExportData) throws {
        ...
        let paymentMapping = try importPayments(importData.payments, memberMapping: memberMapping)
        try importTransactions(importData.transactions, memberMapping: memberMapping, paymentMapping: paymentMapping)
    ...
    private func importPayments(_ payments: [ExportPayment], memberMapping: [UUID: Member]) throws -> [UUID: Payment] {
        ...
        var paymentMapping: [UUID: Payment] = [:]
        ...
        for exportPayment in payments {
            ...
            if let paymentID = exportPayment.paymentID {
                paymentMapping[paymentID] = payment
            }
        }
    ...
    private func importTransactions(_ transactions: [ExportTransaction], memberMapping: [UUID: Member], paymentMapping: [UUID: Payment]) throws {
        for exportTransaction in transactions {
            ...
            // Link to payment (preserves Payment↔Transaction relationship across export/import)
            if let paymentID = exportTransaction.paymentID,
               let payment = paymentMapping[paymentID] {
...
struct ExportTransaction: Codable {
    ...
    let createdAt: Date?
    let updatedAt: Date?
    /// Set when this transaction was generated by a `Payment`. Restoring
    /// the inverse relationship on import prevents `createMissing
    /// Transactions` from silently creating duplicates after a fresh
    /// import (because the previous format dropped this link, every
    /// imported Payment looked like an orphan and got an extra
    /// transaction).
    let paymentID: UUID?
}
